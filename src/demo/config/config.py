...
class AppConfig(BaseSettings):
    ...
    environment: Literal["development", "staging", "production"] = Field(
        default="development", description="Runtime environment", alias="ENVIRONMENT"
    )
    ...
    langsmith_api_key: Optional[str] = Field(
        default=None, description="LangSmith API key for tracing (optional)", alias="LANGSMITH_API_KEY"
    )

    langsmith_project: str = Field(
        default="performance-review-ai", description="LangSmith project name", alias="LANGSMITH_PROJECT"
    )

    # ========================================
    # Model Configuration
    # ========================================
    model_name: str = Field(default="gpt-4o", description="OpenAI model name", alias="MODEL_NAME")
    ...
    max_tokens: int = Field(
        default=10000, gt=0, le=128000, description="Maximum tokens per request", alias="MAX_TOKENS"
    )

    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum retry attempts for LLM calls", alias="MAX_RETRIES"
    )

    request_timeout: float = Field(
        default=60.0, gt=0, description="Request timeout in seconds", alias="REQUEST_TIMEOUT"
    )
    ...
    prompts_dir: Path = Field(
        default=Path("src/prompts"), description="Directory containing prompt templates", alias="PROMPTS_DIR"
    )

    data_dir: Path = Field(default=Path("data"), description="Directory for input/output data", alias="DATA_DIR")

    output_dir: Path = Field(default=Path("data/output"), description="Directory for output files", alias="OUTPUT_DIR")

    # ========================================
    # Performance Review Settings
    # ========================================
    enable_prompt_cache: bool = Field(
        default=True, description="Cache loaded prompts for performance", alias="ENABLE_PROMPT_CACHE"
    )

    enable_fact_checking: bool = Field(
        default=True, description="Enable fact-checking loop", alias="ENABLE_FACT_CHECKING"
    )

    max_fact_check_revisions: int = Field(
        default=3, ge=1, le=10, description="Maximum fact-check revision iterations", alias="MAX_FACT_CHECK_REVISIONS"
    )

    min_bullets: int = Field(
        default=3, ge=1, description="Minimum number of bullet points required", alias="MIN_BULLETS"
    )

    min_ratings: int = Field(
        default=2, ge=0, description="Minimum number of rated bullet points required", alias="MIN_RATINGS"
    )
    ...
    def validate_production_settings(self):
        ...
        if self.environment == "production":
            ...
            if self.temperature > 0.5:
                logger.warning(
                    f"High temperature ({self.temperature}) in production. This may lead to inconsistent results."
                )

            if not self.langsmith_api_key:
                logger.warning("LangSmith not configured for production. Observability will be limited.")
...
def get_config(force_reload: bool = False) -> AppConfig:
    ...
    if _config_instance is None or force_reload:
        ...
        logger.info(
            f"Configuration loaded: environment={_config_instance.environment}, model={_config_instance.model_name}"
        )

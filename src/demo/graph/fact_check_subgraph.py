...
from demo.graph.consts import NODE_FACT_CHECKER_CLAIM_EXTRACTOR, NODE_FACT_CHECKER_REWRITER
...
def create_fact_check_subgraph():
    ...
    subgraph_builder.add_node(NODE_FACT_CHECKER_REWRITER, fc_rewriter)

    subgraph_builder.add_edge(START, NODE_FACT_CHECKER_CLAIM_EXTRACTOR)
    subgraph_builder.add_conditional_edges(
        NODE_FACT_CHECKER_CLAIM_EXTRACTOR, check_fc_result, {"True": NODE_FACT_CHECKER_REWRITER, "False": END}
    )
    subgraph_builder.add_edge(NODE_FACT_CHECKER_REWRITER, END)
